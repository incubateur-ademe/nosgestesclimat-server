...
router.route('/:id?').get((req, res, next) => {
    ...
    initDatabase_1.default.then((db) => {
        const data = SimulationSchema_1.Simulation.find({ id: req.params.id });
...
router.route('/').post(async (req, res, next) => {
    ...
    const found = await SimulationSchema_1.Simulation.find({ id: req.body.id });
    const simulation = found.length
        ? found[0]
        : new SimulationSchema_1.Simulation({ id: req.body.id });
    // @ts-ignore
